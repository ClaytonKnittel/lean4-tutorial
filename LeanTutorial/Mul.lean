...
theorem succ_mul (a b : ℕ) : a.succ * b = a * b + b := by
  rw [mulNat]

theorem mul_succ (a b : ℕ) : a * b.succ = a * b + a := by
  induction a with
  | zero =>
    repeat rw [zero_mul]
    rw [add_zero]
...
theorem one_mul (m : ℕ) : one * m = m := by
  rw [succ_mul, zero_mul, zero_add]

theorem mul_one (m : ℕ) : m * one = m := by
...
theorem two_mul (m : ℕ) : two * m = m + m := by
  rw [succ_mul, one_mul]
